...
class MathGradingProcessor:
    ...
    def __init__(self, output_dir="results", metadata_dir="metadata"):
        """
        Args:
    ...
    def _load_existing_metadata(self) -> None:
        ...
        """
        logger.info(f"기존 메타데이터 로드 중: {self.metadata_dir}")
        for metadata_file in self.metadata_dir.glob("*_metadata.json"):
            try:
                ...
                logger.info(f"  ✓ 로드 완료: {problem_id}")
    ...
    def process_single_solution(self, student_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            analysis, success = service_analyze(problem_id, solution_filename, metadata)
    ...
    def run(self) -> None:
        ...
        if not problems_list:
            logger.error("처리할 문제가 없습니다. 'resource/list.csv'를 확인하세요.")
            return
...
def main() -> None:
    ...
    전제 조건:
        - 'metadata/' 디렉토리에 각 문제별 메타데이터 파일이 존재해야 합니다.
        - 메타데이터는 extract_metadata.py를 실행하여 미리 추출되어 있어야 합니다.

    실행 과정:
        1. 타임스탬프 기반의 결과 디렉토리를 생성합니다.
        2. MathGradingProcessor를 초기화하며 메타데이터를 로드합니다.
        3. resource/list.csv의 모든 학생 풀이를 순회하며 자동 채점을 실행합니다.
        4. 채점 결과를 JSON 파일 형식으로 저장합니다.
    """
